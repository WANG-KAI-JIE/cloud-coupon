...
public class CanaryRule implements ReactorServiceInstanceLoadBalancer {
	...
	public CanaryRule(ObjectProvider<ServiceInstanceListSupplier> serviceInstanceListSupplierObjectProvider, String serviceId) {
	...
	Response<ServiceInstance> getInstanceResponse(List<ServiceInstance> instances,Request request){
		...
		if (StringUtils.isBlank(trafficVersion)) {
			...
			List<ServiceInstance> noneCanaryInstances = instances.stream()
					.filter(e -> !e.getMetadata().containsKey(TRAFFIC_VERSION)).collect(Collectors.toList());
			return getRoundRobinInstance(noneCanaryInstances);
		}

		// 找出金丝雀服务器，用RoundRobin算法挑出一台
